...
// ⚠️ Relative with the extension, like every other import here — this module is
// loaded by plain Node during the prerender pass and by scripts/og-images.mjs.
// notices/index.js was converted off the "@/" alias for exactly this import.
// No cycle: notices/index.js imports only statutory.js, never this file.
import { noticesWithDetail, noticeDetailPath } from "./notices/index.js";
...
export const dscPanelColumns = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        // 20-09-2026 (Clinton): navbar label only — the page's own
...
// ⛔ 20-09-2026 (Clinton): "in details page show the details that mention in
// pdf." A notice can now carry a page of its own; one does today.
//
// ⚠️ DERIVED FROM notices/index.js, the same arrangement `insightArticlePages`
// uses and safe for the same reason — that module imports nothing from here,
// so there is no cycle. Deriving is what keeps the route, the breadcrumb, the
// footer-free sitemap entry and the board row's own href moving together when a
// slug or a headline changes. `noticesWithDetail()` filters on `confirmed`, so
// an unpublished notice gets no route either.
//
// ⚠️ LABELS ARE THE NOTICE'S OWN HEADLINE, read from content rather than
// retyped, so a retitled notice cannot leave the breadcrumb or sitemap
// asserting the old one. The label is long; that is correct — it is the page's
// real name, and insights article routes do the same.
//
// ⚠️ ITS OWN TEMPLATE, T18. Every other id in routeComponents.js resolves
// UNCONDITIONALLY to one component, so reusing one would serve the wrong page
// under this URL in BOTH the client bundle and the prerendered HTML, with
// nothing failing and nothing logging. `lib/seo.js` needs the matching case or
// the page inherits another page's title. Grep `case "T` before claiming a new
// id — T16/T17 collided once — and remember T14 is RETIRED.
export const noticeDetailPages = noticesWithDetail().map((notice) => ({
  slug: notice.detail,
  path: noticeDetailPath(notice.detail),
  label: notice.title ?? notice.label,
  template: "T18",
  parent: noticesPage.path,
}));
...
const slugIndex = new Map([
  ...
  ...noticeDetailPages.map((page) => [page.slug, page]),
