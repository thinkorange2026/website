...
export function resolveSeo(path) {
  ...
  switch (route?.template) {
    ...
    // T18 — a notice's own page. Own case, not a fall-through to T17: that
    // would give every notice detail the LIST page's title and description.
    // ⚠️ `meta` is read off the notice entry in notices/index.js, never from
    // notices/details.js — that file holds the prose, and resolving a <title>
    // must not depend on it.
    case "T18": {
      const notice = slug ? noticesFor("all").find((n) => n.detail === slug) : undefined;
      m = notice?.meta ?? fallbackFor(route);
      break;
    }
