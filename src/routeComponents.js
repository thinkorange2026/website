...
export function resolveComponent(entry, components) {
  ...
  switch (entry.template) {
    ...
    case "T17":
      return Notices;
    // T18 — /notices/:slug, a notice's own page (20-09-2026). Own branch for
    // the same reason every id since T11 has one: each of the others resolves
    // UNCONDITIONALLY, so reusing one — T17 most temptingly, since this route
    // sits directly under /notices — would serve the LIST page under a notice's
    // URL, in the client bundle AND the prerendered HTML, with nothing failing
    // and nothing logging.
    case "T18":
      return NoticeDetail;
    case "T6":
