...
## /dsc/resources: two tools, then the document matrix — 20-09-2026
NOT a phase. Two instructions, one page. The page had shipped EMPTY since
07-09-2026 and now renders its first content.

### The two tool rows
Clinton supplied both verbatim: the **SignX Download Utility** (external, at
`https://ekyc.signxca.com/SignX-Utility-v1.1.18.exe`) and a **Token Drivers**
pointer to `/dsc/drivers` — "link to your existing /dsc/drivers page instead of
duplicating it".
- ⛔ **"SignX" IS BACK ON THE SITE, in this one place.** On 02-09-2026 Clinton
  said "do not use signx it is for the other company name" and 45 mentions came
  out. This title is back because he supplied it and the URL on 20-09-2026, and
  because it is a different kind of claim — it NAMES A FILE someone installs,
  not a partnership. **The policy is unchanged everywhere else** (still "a
  licensed Certifying Authority"), and this entry is not licence to reintroduce
  a name anywhere else. The host is in the URL regardless, so a neutral title
  would hide the name from the copy and not from the link.
- ⚠️ **We do not host the binary, deliberately.** CONTENT-PLAN.md §9 says a
  vendor binary needs its redistribution terms checked before it is hosted here;
  linking the vendor's own URL sidesteps that. It also means there is no
  checksum to publish — we cannot vouch for bytes we do not serve, which is why
  the row says "Hosted by the certifying authority".
- ⚠️ **The URL is VERSION-PINNED.** A new vendor build at a new filename breaks
  the link AND makes the rendered "Version 1.1.18" stale at the same moment.
  Re-check both together; the version is read off the filename, not asserted
  from anywhere else. `curl` returns 403 `cf-mitigated: challenge` — Cloudflare
  blocking a non-browser client, not a dead link, but it does mean the file was
  never confirmed from the CLI.
- ⛔ **THREE ROW SHAPES IN `DscResources.jsx`, and the differences are not
  cosmetic.** An internal destination is a react-router `<Link>` — an
  `<a download>` there hands the browser an HTML page to save, and a plain `<a>`
  loses in-app navigation. A file we do NOT host **drops** `download` (browsers
  ignore it cross-origin, so keeping it claims behaviour the link does not have)
  and takes `rel="noopener noreferrer"`. A file we host keeps `download`.
- The meta line renders only when it has content — a row with no
  format/size/version would otherwise sit a line lower than its neighbours, the
  "heading over nothing" defect class.
- **Page copy widened** from "forms, checklists and reference documents" to
  cover utilities and drivers: the page had neither a form nor a checklist on
  it, and a heading asserting content that is not below it is exactly what this
  repo keeps getting bitten by.

### The document matrix
Clinton: "analyse https://www.signxca.com/resources.php … show the resource
details like this … keep the design like the tools and driver download. show
some details and + on right side while click expand and show the whole
details." Read off that page's DOM (all 32 combinations are in it, `d-none`
toggled), not from its screenshot.
- **`src/content/dsc/document-matrix.js`** — 8 organisation types plus a shared
  address-proof row. Each type answers three things separately:
  **organisation / authorised signatory / applicant**, which is the structure a
  CA actually checks and the thing a single rolled-up list destroys.
- ⛔ **THE SUBSTANCE IS THEIRS, THE WORDING IS OURS.** Which documents a CA
  requires per entity type follows from CCA guidance and is a fact; the
  sentences on their page are their expression of it, and copying them wholesale
  would put a third party's copy on a client site. Every line was rewritten —
  sentence case, -ise spelling, their typos gone ("Authroised", "orgnaization"),
  and every "NA" turned into a real statement of what is not required. **If you
  re-check this against their page, compare the REQUIREMENTS, not the strings.**
- ⛔ **NO CERTIFYING AUTHORITY IS NAMED IN THE MATRIX.** Every reference is "a
  certifying authority". The utility row above names one because the product is
  called that; this content has no such excuse.
- ⚠️ **It does not contradict `certificates.js`, and that was checked rather
  than assumed.** Those per-certificate lists are the short "what we ask you
  for"; this is the fuller picture a CA works from, adding the alternatives they
  roll up. **If a line ever does disagree, `certificates.js` wins.**
- ⚠️ **SHAPE: each of the three columns is a list of `{ when?, items }`
  GROUPS**, so a conditional requirement ("if the signatory is a partner… if
  not…") and a plain list are the same structure with no special case in the
  template. `null` means genuinely not required and the panel says so — a
  column that rendered empty would read as content that failed to load.
- **`Disclosure` is reused**, not reinvented: the same open-one-at-a-time row
  with the `+` rotating to `×` that /dsc/drivers and /dsc already use, which is
  what "keep the design like the driver download" actually means. Bullets match
  `DriverPanel`'s established light-surface marker (`h-1.5 w-1.5 bg-ember-500`)
  rather than a second, fainter one.
- ⚠️ **Panels stay MOUNTED while collapsed**, which is what puts all eight
  types in the prerendered HTML — the crawlability property `Disclosure`'s own
  header records. It also means a link inside a panel would stay in the tab
  order: **there are none here and there must not be.**
- Icons resolve through `typeIcon()` **with a fallback**, never by indexing the
  map — an unmapped key gives `<undefined />`, a hard React crash, which has
  shipped from a slug-keyed icon map here once already.
- ⚠️ Four sign-off items in MISSING-PAGES.md, none blocking: the matrix needs a
  CA's confirmation; the source asks for a "list of directors" for an **LLP**
  (whose register names designated partners) and that is reproduced as given;
  it still lists a **service tax / VAT / sales tax** certificate as an address
  proof, which GST subsumed in 2017; and the vendor also ships a macOS
  `arm64.pkg` build that we do not link, so a Mac client has no download.

### Verified
`npm run lint` 0 errors (the 4 warnings are pre-existing, in `DscEsign.jsx` and
`DscHub.jsx` from other sessions), `build` + prerender clean with the
dangling-fragment and og:image gates passing, and a byte-level read of
`dist/dsc/resources/index.html`: both tool rows with the right hrefs, `rel` and
`download` attributes, all 8 types and the address-proof list present as real
text, and no `undefined` / `[object Object]` / `NaN`.

Real Chrome over CDP against `npx serve dist` (never the in-app pane — it
suspends IntersectionObserver, so every `Reveal` renders at opacity 0 and the
page looks blank), asserting `innerWidth`/`visibilityState`/`pathname` first and
priming reveals with awaited 400px steps from Node: 9 rows with the right
labels, cadence **`deep → light → light-alt → ember`** (zero consecutive
repeats, no adjacent dark-family pair), `scrollWidth === innerWidth` at 1440 and
375, a row opening to a 365px panel (807px at 375) with its real text, the `+`
at `rotate: 135deg`, and single-open enforced. Contrast computed on the flat
`canvas-alt` surface — nothing translucent over it, so a static pair is valid
here: h4 11.95:1, body 9.12:1, mono group label 6.06:1.
- ⚠️ Probe traps, both re-confirmed: set `Emulation.setDeviceMetricsOverride`
  **before AND after** `Page.navigate`, or `innerWidth` comes back as the
  window's own 756; and reading `aria-expanded` synchronously after a `click()`
  reports the OLD value, because React updates async — assert on the panel
  height and the `+`'s `rotate` instead (and read `.rotate`, never
  `.transform`, since Tailwind v4 compiles `rotate-*` to the individual
  property).
- ⚠️ **Route count read 64, not the 63 this session's earlier build reported.**
  Nothing here added a route; another session's in-flight work did. Left as
  found.

## Notice detail pages (T18) + the emSigner 3.3 notice — 20-09-2026
NOT a phase. Clinton supplied `ThinkOrange Content Draft - emSigner 3.3.pdf`
(marked STATUS: READY FOR PUBLISHING) with three instructions: "i want to keep
this notice at top", "in details page show the details that mention in pdf.
keeps the wording same do not change a single word", and use the graphic he had
already placed at `public/images/notice/notice-token.png`. **64 routes (was
63).**

- **A notice can now carry a page of its own.** `/notices/emsigner-3-3` is the
  first; the mechanism is general. A notice opts in with a `detail` slug and
  **its `href` is then COMPUTED, never authored** (`noticeDetailPath()`), so the
  row, the route and the breadcrumb cannot disagree about where it lives.
  `noticesWithDetail()` filters on `confirmed`, so **`confirmed: false` withholds
  the ROUTE as well as the row** — a hidden notice with a live, crawlable,
  prerendered page would defeat the only flag standing between that file and an
  unreviewed claim.
- ⛔ **`src/content/notices.js` IS NOW A DIRECTORY**, `notices/index.js` +
  `notices/details.js`, exactly the `insights/index.js` ↔ `insights/bodies.js`
  split and for the same measured reason: `nav.js` imports the index to derive
  these routes and `nav.js` is in the always-eager main chunk, so the prose
  would have been paid for on every route. **Verified in the built output: the
  body text appears in `NoticeDetail-*.js` and in no other chunk.**
  ⚠️ `title` and `meta` live on the notice entry in `index.js`, NOT in
  `details.js` — nav.js derives the route label from one and `lib/seo.js`
  resolves the `<title>` from the other, and neither may reach into the prose
  file to get them. Same reason insights keeps them split.
- ⛔ **`notices/index.js` had to come off the `@/` alias** (`import { s } from
  "../statutory.js"`). CLAUDE.md already records that this file "gets away with
  the alias only because nothing Node-side imports it" — nav.js imports it now,
  and nav.js is loaded by plain Node during the prerender pass and by
  `og-images.mjs`, where the alias does not resolve. No cycle: notices/index.js
  imports only statutory.js.
- ⚠️ **ITS OWN TEMPLATE, T18.** Grep `case "T` before claiming an id — T16/T17
  collided once in this repo and nothing failed, one route simply rendered the
  other's component. **T17 was the tempting reuse here**, since this route sits
  directly under `/notices`, and it would have served the LIST page under a
  notice's URL in the bundle AND the prerendered HTML. Wired in all four places:
  `routeComponents.js`, `router.jsx`, `router-static.jsx`, `lib/seo.js`.
  **T14 is still retired and must not be reused.**

### The copy is verbatim, and the enforcement date still is not typed
- `details.js` carries the draft word for word — American spellings
  ("recognize"), unspaced em dashes, US date format, all as written. The file's
  header says so and says not to house-style it.
- ⛔ **THE DATE IS STILL INTERPOLATED.** New key
  `statutory.js → emsigner33GstEnforcement`. ⚠️ It holds the **same underlying
  date as `fips1403DscIssuance` in a different typography**, and that is the ONLY
  reason two entries exist — the supplied copy writes "September 21, 2026" and
  reformatting it inside `s()` would have broken the instruction. **Correct both
  together; both entries cross-reference each other.**
  - The draft names the date twice, the second time without the year. The short
    form is **DERIVED** (`replace(/,\s*\d{4}$/, "")`), not typed as a second
    literal, so one correction reaches both sentences.
  - ⚠️ Its `source` is the client's own draft — the only non-link `source` in
    that file. Logged in MISSING-PAGES.md.
- **Product specs are NOT statutory values and stay literal**: Windows/macOS/
  Ubuntu versions, 8 GB RAM, Java 1.8. Same category as `drivers.js`'s
  `supportedOs` and the HYP2003 spec table. statutory.js is for rupee amounts,
  day counts, form codes, penalties, thresholds and deadlines.
- **Bold runs inside sentences are preserved** via a segment shape
  (`[{ text, strong }]`), the same form `Typewriter`'s headline lines use —
  emphasis is part of the copy that was asked to be kept, and a `**markdown**`
  string would have meant writing a parser. Verified all four bold phrases
  render exactly as drafted.
- ⛔ **THE DRAFT'S CLOSING "Need Technical Support?" PANEL IS NOT REPRODUCED, and
  that is not an oversight.** Its second line is **corrupted in the source PDF** —
  the export embedded only a handful of glyphs (`l i Si J i ll GST li h`) and the
  panel is visually clipped by the page boundary, so it cannot be read off a
  render either. Confirmed three ways: pypdf text, pypdf layout mode, and pdfium
  character boxes. Recoverable text stops mid-clause at "...configuring your FIPS
  140-3 tokens,". Finishing it would be writing copy and attributing it to
  Clinton, so the page closes with `CtaBand` instead. **Ask him for that
  sentence** — MISSING-PAGES.md carries it.

### Template decisions
- **`PageHero` (dark), not the article template's light editorial header.** This
  page opens under the fixed transparent header like ~40 others, so it needs no
  `lightTop` entry in nav.js and no header change; and a notice is an alert
  rather than an essay. Cadence `deep → light → ember`.
- ⚠️ **The body is TWO COLUMNS.** A lone 68ch measure left the right half of the
  1800px container empty — the "empty half" fault `FaqSection`, `StepFlow` and
  the /contact rebuild each exist to fix. The rail carries the article
  template's own "Rather just ask?" card, **lifted verbatim rather than drafted**
  so the two long-form templates offer the same escape hatch in the same words.
  Site chrome, not notice copy, which is why writing it does not breach the
  instruction. Same `lg:top-32` sticky offset.
- ⚠️ **The graphic's `ratio` is the file's REAL pixel ratio (1672/941).**
  `<Img>`'s inner img is `object-cover`, which only leaves the artwork uncropped
  while the box matches the file's own aspect — and this one carries text, so a
  "nicer" design ratio would silently crop words off it. `priority`, no `Reveal`:
  it is the page's LCP element.
  - The `src` is `/images/notice/...`, **never `public/images/...`** — `public/`
    is Vite's build root, not a URL segment. That bug has shipped here once.
  - ⚠️ **The `alt` TRANSCRIBES the graphic, so its date is a LITERAL and must not
    be interpolated** — the date is baked into the PNG, and pointing it at
    statutory.js would let the words disagree with the picture.
- The "Pre-Installation Checklist" is a **`<dl>`, not a `<table>`**: the data is
  term → detail pairs, which a description list expresses just as correctly and
  which needs no `overflow-x-auto` + `min-w-*` pair to survive 375px. The /dsc
  comparison table had to be rebuilt to stop a phone reader scrolling sideways;
  this avoids the problem rather than solving it twice.
- The callout's ⚠ character is a real icon, `aria-hidden`, so the heading's
  accessible name stays "Note for Early Adopters" rather than "warning sign Note
  for Early Adopters". Verified off the rendered DOM.
- ⚠️ `bg-ember-950` still does not exist (the ramp stops at 700) — the callout is
  `bg-ember-50`. A class for an undefined token compiles to nothing, with no
  error.

### Verified
`npm run lint` 0 errors (4 pre-existing warnings, in `DscEsign.jsx` and
`DscHub.jsx` from other sessions), `content:check` clean apart from the standing
testimonial blocker, `build` + prerender **64 routes + 14 redirects** with the
og:image and dangling-fragment gates passing, link integrity over `dist/`
**3,339 internal refs / 0 broken**.

A byte-level check of the prerendered HTML asserts **all 21 of the draft's
strings present verbatim**, one `<h1>`, 4 ld+json blocks
(`Organization + LocalBusiness + BlogPosting + BreadcrumbList` — one
BreadcrumbList, since `PageHero` emits it), and no `undefined` / `[object
Object]` / `NaN`. The notice is **first** on `/notices`, on `/dsc/faqs` and in
the homepage ticker.

Then a real Chrome over CDP against `npx serve dist`, asserting
`innerWidth`/`visibilityState`/`pathname` first: **pixel-sampled contrast 0
failures** — 33 samples at 1440px (tightest 4.97:1), 32 at 375px (5.07:1), and
/notices re-checked at 0/15; reduced motion via `Emulation.setEmulatedMedia`
**0 running animations, 0 elements stuck mid-opacity**; `scrollWidth <=
innerWidth` at both widths; and a six-route regression across every template
family with **0 console errors**.

### ⚠️ A THIRD contrast-harness trap, on top of the two already recorded
The sweep reported 3 failures at ~2.04:1 on plain `text-ink-500` body copy —
including on `/notices`, which this change never touched. All artifacts: a
direct measurement of one flagged paragraph read the background as a uniform
`rgb(251,249,245)` and the pair at **9.78:1**.

Cause: the harness read element RECTS before injecting the `color: transparent`
probe, then captured after settling — so boxes and pixels described **different
layouts**, because `Reveal` was still translating elements 16px. CLAUDE.md
already records "read the FOREGROUND COLOUR before injecting" and "inject the
probe as its own step and settle before capturing"; this is a third, distinct
requirement:

⛔ **Read colours BEFORE the probe, but RE-READ THE RECTS AFTER it has settled**,
and settle long enough (~1.2s post-scroll) for reveals to finish. With that, the
same run reported **0 failures**. Tag elements with a `data-` index on the first
pass so the two reads can be joined.

Also: filtering `opacity < 0.99` on the ELEMENT is not enough — the opacity lives
on the `Reveal` WRAPPER, so check the rect-time computed opacity too.

### Follow-up: the featured notice as a clickable bar under the homepage hero — 20-09-2026
Clinton: "show this notice in bar also. after hero section. show this one olny.
while click redirect to details page."

- **`NoticeBar` (ONE notice, by prop), not `NoticeTicker` (every `scope: "site"`
  notice)** — "show this one olny". The ticker stays commented out on the
  homepage; turning it back on would put two marquees under one hero.
- ⚠️ **THE HOMEPAGE NAMES NO SLUG.** A notice opts in with **`featured: true`**
  and `featuredNotice()` returns it with its DERIVED href, so moving or
  retiring the feature is a one-line content edit and nothing in
  `home/index.jsx`. `confirmed: false` withholds it too, and `NoticeBar` returns
  null without `text`, so the page degrades to hero → calendar with the cadence
  intact. **At most one notice may carry the flag** — the helper returns the
  first and warns in dev about the rest, because a second one silently not
  appearing looks like the flag being broken rather than like a taken slot.
- ⛔ **`href` MAKES THE WHOLE STRIP THE TARGET, never the moving words.** The
  track is DUPLICATED for the loop, so a link inside it would exist twice in the
  tab order pointing at one place — `aria-hidden` silences a screen reader but
  does NOT remove anything from the tab order — and a target physically sliding
  out from under the cursor is hostile anyway. `NoticeTicker`'s header refuses
  links for exactly this reason; wrapping the WINDOW rather than the content is
  what sidesteps it. Measured: **1 link, 1 focusable, 1 sr-only copy**, and the
  link's accessible name is the notice in full.
- ⚠️ **The focus ring is INSET.** The section is the `overflow-hidden` marquee
  window, so an ordinary outline on a child filling it is clipped away and the
  bar looks unfocusable. Verified `inset 2px` ember under focus emulation.
- Cadence `deep → light-alt → light`, zero repeats, zero adjacent dark pairs.
  Clicking it lands on `/notices/emsigner-3-3`.

### ⛔ Two real defects this surfaced, both fixed
1. **THE MARQUEE LOOP WAS BROKEN ON ANY MONITOR WIDER THAN ~2600px, and the old
   note in the file gave false comfort.** `translateX(-50%)` travels exactly one
   group, so a group must be at least as wide as the window — but **the type
   scale caps at 1800px (theme.css's xl step-up), so group width stops growing
   while the viewport does not.** Measured at 2 passes with this notice:
   **2659.5px against a 2560px window — 99px of headroom, and negative beyond
   that**, which shows as a strip of empty ember sliding in at the right edge
   before the snap back. The previous comment claimed ~1100px spare; that was
   true of the *HYP2003 sentence*, not a property of the component.
   **`GROUP_PASSES` is now 4.** Re-measured: group 5319px at every width ≥1800,
   so it clears 5K (5120px); `/dsc/buy-token` goes to 6962px. Cost is 4 extra
   spans and a longer loop PERIOD only — **speed is measured and stayed exactly
   55px/s at 375/1440/2560/3440/3840/5120**.
2. ⛔ **The 11px label failed AA in the HOVER state, which the resting
   measurement hid.** ember-600 is 4.79:1 on the bar's ember-50 ground — the
   documented pass — but only **4.04:1** once a linked bar deepens to ember-100,
   under the 4.5:1 floor. **WCAG applies to every state; a resting pass is not
   enough for an interactive surface.** Label and both glyphs are now ember-700:
   **6.64:1 at rest, 5.60:1 on hover.** Applied to BOTH instances rather than
   only the linked one — a third label colour inside one component is drift, and
   it strictly improves the unlinked bar too. `NoticeBoard` is untouched: its
   rows sit on WHITE, a different measurement.
   ⚠️ Measuring a Tailwind `/60` alpha variant by compositing it on a canvas
   does NOT work — v4 emits `oklab()`/`oklch()` with alpha, which `fillStyle`
   silently refuses, so the canvas keeps the previous colour and every candidate
   reports the base value. The opacity candidates in that sweep were fiction;
   the two solid colours were not.

### Verified
`npm run lint` 0 errors (4 pre-existing warnings), `content:check` clean apart
from the standing testimonial blocker, `build` + prerender 64 routes + 14
redirects with both gates passing. Real Chrome over CDP against `npx serve
dist`: loop invariant at six widths and both call sites; click-through landing
on the detail page; **contrast 0 failures on `/dsc/buy-token` (125 samples) and
`/notices/emsigner-3-3` (33 @1440, 32 @375)**; reduced motion **0 running
animations, 0 marquee elements, static copy still linked**; six-route
regression with **0 console errors** and cadence unchanged everywhere.

### ⚠️ Pre-existing, NOT from this change, and a genuine AA failure
The homepage Insights FEATURE PANEL's eyebrow — `text-ember-200`, 11px, over the
photograph — measures **1.54:1 at 375px**. CLAUDE.md records that panel's eyebrow
at 4.83:1, but **that figure was taken at desktop**; at phone width the visible
slice of the photo under the text is a mid-grey (145,148,157) and the scrim does
not cover it.

**Confirmed pre-existing by rebuilding with the bar removed: byte-identical
1.54:1.** Not touched, because it is a different section on a different concern —
but it is a real failure and the fix is small (deepen the scrim's hold at mobile,
